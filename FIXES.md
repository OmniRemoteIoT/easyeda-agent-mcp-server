...
| **1.3.1** | **Layer name→id on the READ side too (below)**: the `pcb.getAll.*` filters (line/polyline/arc/pad/region/pour/fill) also passed the layer NAME, so `pcb_get_all_primitives(layer:"Board Outline Layer")` returned `[]` even when layer-11 primitives existed (matched name vs stored id 11). Applied `resolvePcbLayer()` to the getAll layer filters too. `.eext` re-import | ext | ⏳ |
| **1.3.0** | **PCB primitive creates used the layer NAME, not the id — THE board-outline bug (below)**: `pcb_create_track`/`polyline`/`arc`/`pad`/`region`/`pour`/`fill` passed the layer as a NAME string (e.g. "Board Outline Layer") to the create APIs, which require the numeric layer **id** (11 = outline). EDA stored the malformed primitive (layer as a string) so it saved + read back in-session, but it NEVER rendered and wasn’t recognized (board-outline catalog = 0). Confirmed by decoding the saved .eprj: real LINEs store layer as a number (1/2), mine stored `"Board Outline Layer"`. Fix: new `resolvePcbLayer()` maps layer name→id (via `pcb_Layer.getAllLayers()`, cached, space/case-insensitive) and every create handler resolves it. Fixes ALL PCB primitive creation, not just outlines. `.eext` re-import + restart | ext | ⏳ live re-test |
